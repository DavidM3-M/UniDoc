<?php

namespace App\Models\Docente;

use App\Models\Usuario\User;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;

class Puntaje extends Model
{
    protected $table = 'puntajes';
// Especifica el nombre de la tabla asociada con este modelo en la base de datos.
    protected $primaryKey = 'id_puntaje';
    // Define la clave primaria de la tabla como `id_puntaje`.

    protected $fillable = [
        'user_id',
        'puntaje_total',
        // Caché denormalizada del escalón vigente en `historial_escalon_docente`, que es la fuente
        // de verdad. La escribe únicamente `AscensoEscalafonService`, para que los listados no
        // tengan que unir contra el historial fila por fila.
        'categoria_lograda',
    ];
    // Define los campos que se pueden asignar masivamente (mass assignment) en este modelo.

    public function usuarioPuntaje(): BelongsTo
        // Define una relación entre el modelo `Puntaje` y el modelo `User`.
    {
        return $this->belongsTo(User::class, 'user_id', 'id');
         // Establece una relación de uno a muchos inversa entre `Puntaje` y `User`.
        // Usa la clave foránea `user_id` en la tabla `puntajes` y la clave primaria `id` del modelo `User`.
    }
}

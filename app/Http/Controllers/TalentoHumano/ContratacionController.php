...
class ContratacionController
{
    ...
    public function eliminarContratacion($id)
    {
        ...
        // Caso opcional de la eliminación: 'desvinculacion' | 'contrato_no_iniciado' | null (genérico).
        ...
        try {
            ...
            DB::transaction(function () use ($id, $motivo, &$usuarioAfectado) {
            ...
            // Notificar por correo el motivo de la eliminación (fuera de la transacción,
            // para no enviar correo si la operación se revierte).
            try {
                if ($usuarioAfectado) {
                    NotificacionController::contratacionEliminada($usuarioAfectado, $motivo, $caso);
                }
            } catch (\Exception $notifEx) {
                Log::error('Error al notificar eliminación de contratación: ' . $notifEx->getMessage());
            }

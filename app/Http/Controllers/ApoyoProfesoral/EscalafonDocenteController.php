...
class EscalafonDocenteController
{
    ...
    public function crearPeriodo(CrearPeriodoAscensoRequest $request)
    {
        try {
            ...
            $this->anunciarPeriodoAbierto($periodo);
    ...
    /**
     * C3 — anuncia el periodo recien abierto a los docentes que pueden aspirar a algo.
     *
     * Se excluye a quien no esta dentro del escalafon —el aviso no le dice nada que pueda hacer— y
     * a quien ya esta en el escalon maximo, que no tiene a donde ascender. Es el unico correo del
     * sistema que va al padron casi completo, asi que conviene que no llegue a quien le sobra.
     */
    private function anunciarPeriodoAbierto(PeriodoAscenso $periodo): void
    {
        $topeOrden = EscalonDocente::activos()->max('orden');

        $docentes = User::role('Docente')
            ->whereHas('historialEscalonUsuario', function ($q) use ($topeOrden) {
                $q->whereNull('hasta')
                  ->whereNull('revertido_en')
                  ->whereHas('escalon', fn ($e) => $e->where('orden', '<', $topeOrden));
            })
            ->get();

        $plazo = (int) now()->startOfDay()->diffInDays($periodo->fecha_cierre, false);

        foreach ($docentes as $docente) {
            EnviarNotificacionJob::dispatch(
                "escalafon.periodo-abierto:periodo:{$periodo->id_periodo_ascenso}:user:{$docente->id}",
                'escalafon.periodo-abierto',
                'periodoAscensoAbierto',
                [$periodo->nombre, $periodo->fecha_cierre->format('d/m/Y'), $plazo],
                $docente->id
            );
        }
    }

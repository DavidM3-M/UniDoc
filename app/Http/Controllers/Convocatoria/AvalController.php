    ...
    public function avalHojaVida(Request $request, $userId)
    {
        try {
            ...
            DB::transaction(function () use ($request, $user, $role, $convocatoriaId, $avalNombre) {
                ...
                // Verificar el estado actual del aval para esta convocatoria
                ...
                if ($avalActual) {
                    // Ya fue aprobado: no se registra de nuevo
                    if ($avalActual->estado === 'aprobado') {
                        throw new \Exception("El aval de {$role} ya fue registrado para esta convocatoria.", 409);
                    }

                    // Ya fue rechazado: no se puede otorgar aval sobre una decisión rechazada
                    if ($avalActual->estado === 'rechazado') {
                        throw new \Exception(
                            "El aval de {$role} ya fue rechazado para esta convocatoria. "
                            . "No se puede otorgar un aval sobre una decisión ya rechazada.",
                            409
                        );
                    }
                }
